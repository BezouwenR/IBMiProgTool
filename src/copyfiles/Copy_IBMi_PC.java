...
public class Copy_IBMi_PC {
      ...
      try {
         ...
         if (sourcePathString.startsWith("/QSYS.LIB/")) {
            ...
            if (sourcePathString.endsWith(".MBR")) { //
              ...
              // ---------            
            else if (ifsDirFile.toString().contains(".LIB")
                     && ifsDirFile.toString().endsWith(".FILE")
                     && ifsDirFile.getSubtype().equals("SAVF")) {
   ...
   protected String copyToPcDirectory(String as400PathString, String pcPathString,
            String ifsPathStringPrefix) {
      ...
      try {
         ...
         if (sourcePathString.startsWith("/QSYS.LIB/")) {
            if (ifsPath.isDirectory()) {
               ...
               return msgText;
            } else if (sourcePathString.endsWith(".FILE") && ifsPath.getSubtype().equals("SAVF")) {
...
*/                     
   ...
   protected String copyFromSourceMember(AS400 remoteServer, String as400PathString, String pcPathString) {
      ...
      // Create new PC file name and path
      String sourceMbrName = memberName;
      String sourceType;
      ...
      if (Files.isDirectory(pcFilePath)) {
         ...
         if (Files.notExists(newPcFilePath) || properties.getProperty("OVERWRITE_FILE").equals("Y")) {
            // New file path is (directory + file) path 
      ...
      // -----------
      {
         ...
         // Note: Check for overwrite is done in the this method
         msgText = copyDataFromMemberToFile(remoteServer, as400PathString, newPcFilePathString);
      }
      return msgText;
   }
    ...
    */
   @SuppressWarnings("UseSpecificCatch")
   protected String copyFromSourceFile(AS400 remoteServer,
            String as400PathString, String pcPathString) {
   ...
   protected String copyFromSaveFile(AS400 remoteServer, String as400PathString, String pcPathString) {
      ...
      try {
         ...
         // 
         if (Files.isDirectory(Paths.get(pcPathString))) {
            filePathString = pcPathString + pcFileSep + fileName + ".savf";
         ...
         if (!Files.exists(Paths.get(filePathString))) {
            ...
            if (!filePathString.endsWith(".savf")) {
               row = "Error: Save file  " + libraryName + "/" + fileName + "  cannot be coppied to the PC file  "
         ...
         row = "Comp: Save file  " + libraryName + "/" + fileName + "  was copied to PC " + directoryOrFile + " " + pcPathString + ".";
         mainWindow.msgVector.add(row);
         mainWindow.reloadLeftSideAndShowMessages(nodes);
         return "";
      } catch (Exception exc) {
   ...
   /**
    * Extract individual names (libraryName, fileName, memberName) from the IBM i IFS path.
    * 
    * @param as400PathString
    */
      ...
      if (as400PathString.startsWith(qsyslib) && as400PathString.length() > qsyslib.length()) {
         ...
         if (as400PathString.length() > qsyslib.length() + libraryName.length() + 5) {
            ...
            if (as400PathString.length() > qsyslib.length() + libraryName.length() + 5 + fileName.length() + 6) {
               memberName = as400PathString.substring(qsyslib.length() + libraryName.length() + 5 + fileName.length() + 6,
