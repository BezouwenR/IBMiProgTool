...
public class Copy_IBMi_IBMi {
   ...
   protected void copy_IBMi_IBMi() {
      ...
      sourcePathStringPrefix = sourcePathString.substring(0, sourcePathString.indexOf(inputDirFile.getName()));
      ...
      try {
         if (inputDirFile.isDirectory() && outputDirFile.isDirectory()) {
            ...
            if (sourcePathString.startsWith("/QSYS.LIB") && targetPathString.startsWith("/QSYS.LIB")) {
               ...
               } else if (inputDirFile.getSubtype().equals("SAVF")) {
         ...
         // File to File
         else if (inputDirFile.isFile() && outputDirFile.isFile()) {
            // From file to file
            if (sourcePathString.startsWith("/QSYS.LIB") && targetPathString.startsWith("/QSYS.LIB")) {
   ...
   protected String copyFromIfsFile(String sourcePathString, String targetPathString) {
      ...
      try {
         ...
         if (targetPathString.startsWith("/QSYS.LIB/")) {
            ...
            if (targetPathString.endsWith(".FILE") && outputDirFile.getSubtype().equals("SAVF")) {
         ...
         if (sourcePathString.startsWith("/QSYS.LIB/")) {
            ...
            if (sourcePathString.endsWith(".MBR")) { //
              ...
              // ---------
                  ...
                  && inputDirFile.toString().endsWith(".FILE")
                  && inputDirFile.getSubtype().equals("SAVF")) {
      ...
      try {
         ...
         if (sourcePathString.startsWith("/QSYS.LIB/")) {
            if (inputDirFile.isDirectory()) {
               ...
               } else if (sourcePathString.endsWith(".FILE")
                     && inputDirFile.getSubtype().equals("SAVF")) {
   ...
   protected String copyToSourceMember(String sourcePathString, String targetPathString) {
      ...
      try {
         ...
         // If *DEFAULT set standard source type, otherwise keep application
         // parameter
   ...
   protected String copyFromSaveFile(AS400 remoteServer, String sourcePathString, String targetPathString) {
      ...
      try {
         ...
         // ----------------
            ...
            // Extract individual names (library, file, member) from the AS400
            // IFS path
            extractNamesFromIfsPath(sourcePathString);
            ...
            // Check if the file to copy already exists and if can be
            // overwritten.
            if (newTargetPath.exists() && !overwriteAllowed) {
               row = "Error: Save file  " + inLibName + "/" + inFileName
                     + "  was NOT copied to existing save file  " + newTargetPath.getName()
   ...
   /**
    * Extract individual names (libraryName, fileName, memberName) from the IBM
    * i IFS path.
    * 
    * @param sourcePathString
    */
   protected void extractNamesFromIfsPath(String sourcePathString) {

      qsyslib = "/QSYS.LIB/";
      if (sourcePathString.startsWith(qsyslib) && sourcePathString.length() > qsyslib.length()) {
         libraryName = sourcePathString.substring(sourcePathString.indexOf("/QSYS.LIB/")
               + 10, sourcePathString.lastIndexOf(".LIB"));
         if (sourcePathString.length() > qsyslib.length() + libraryName.length() + 5) {
            try {
               fileName = sourcePathString.substring(qsyslib.length() + libraryName.length()
                     + 5, sourcePathString.lastIndexOf(".FILE"));
               if (sourcePathString.length() > qsyslib.length() + libraryName.length() + 5
                     + fileName.length() + 6) {
                  try {
                     memberName = sourcePathString.substring(qsyslib.length() + libraryName.length() + 5
                           + fileName.length() + 6, sourcePathString.lastIndexOf("."));
